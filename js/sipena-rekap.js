...
window.renderRekap = () => {
  ...
  const kelasOpts = kelas.map(k =>
    '<option value="' + k.class_name + '"' + (currentRekapClass === k.class_name ? ' selected' : '') + '>' + k.class_name + '</option>'
  ).join('');

  let filterHtml = '<div style="display:flex;gap:12px;flex-wrap:wrap;align-items:flex-end;">' +
    '<div class="fg" style="margin:0;min-width:160px;"><label>Kelas</label><select id="rekapKelasSelect">' + kelasOpts + '</select></div>';

  if (currentRekapTab === 'bulanan') {
    const years = Array.from({ length: 5 }, (_, i) => new Date().getFullYear() - 2 + i);
    const bulanOpts = monthNames.map((m, i) =>
      '<option value="' + (i + 1) + '"' + (selectedMonth === i + 1 ? ' selected' : '') + '>' + m + '</option>'
    ).join('');
    const tahunOpts = years.map(y =>
      '<option value="' + y + '"' + (selectedYear === y ? ' selected' : '') + '>' + y + '</option>'
    ).join('');
    filterHtml += '<div class="fg" style="margin:0;min-width:130px;"><label>Bulan</label><select id="rekapBulanSelect">' + bulanOpts + '</select></div>' +
      '<div class="fg" style="margin:0;min-width:100px;"><label>Tahun</label><select id="rekapTahunSelect">' + tahunOpts + '</select></div>';
  } else if (currentRekapTab === 'semester') {
    const ganjilSel = selectedSemester === 'ganjil' ? ' selected' : '';
    const genapSel = selectedSemester === 'genap' ? ' selected' : '';
    filterHtml += '<div class="fg" style="margin:0;min-width:160px;"><label>Semester</label><select id="rekapSemSelect">' +
      '<option value="ganjil"' + ganjilSel + '>Ganjil (Jul-Des)</option>' +
      '<option value="genap"' + genapSel + '>Genap (Jan-Jun)</option></select></div>' +
...
window.cetakRekap = () => {
  ...
  if (currentRekapTab === 'bulanan') {
    ...
    const semText = semSelect ? (semSelect.value === 'ganjil' ? 'Ganjil (Jul-Des)' : 'Genap (Jan-Jun)') : '';
    tahunText = tahunSemInput ? tahunSemInput.value : new Date().getFullYear();
    periodeText = 'Semester ' + semText + ' Tahun ' + tahunText;
  } else {
