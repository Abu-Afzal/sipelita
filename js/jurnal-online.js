// ══════════════════════════════════════════════
// FIREBASE CONFIG (Firestore only - tanpa RTDB)
// ═════════════════════════════════════════════
...
// ══════════════════════════════════════════════
// INIT - Tunggu Firebase Auth siap (anti race condition)
// ══════════════════════════════════════════════
document.addEventListener('DOMContentLoaded', () => {
    const formJurnal = document.getElementById('formJurnal');
    if (!formJurnal) return;
    
    // ✅ Auth listener tunggal - sinkron sempurna dengan HTML
    auth.onAuthStateChanged(async (user) => {
        ...
        // Ambil data user dari Auth + localStorage (untuk nama lengkap, role, dll)
        let localUser = null;
        try {
        ...
        currentUser = {
            uid: user.uid,
            email: user.email,
            nama: localUser?.nama || user.displayName || user.email.split('@')[0] || 'Guru',
        ...
        if (elDisplay) elDisplay.textContent = '👨‍🏫 Hi, ' + currentUser.nama;
        if (elNamaGuru) elNamaGuru.value = currentUser.nama;
        
        // Load NIP dari localStorage jika pernah diisi
        const savedNip = localStorage.getItem('sipelita_nip_' + sanitizeEmail(currentUser.email));
        if (savedNip) document.getElementById('nipGuru').value = savedNip;
...
// ══════════════════════════════════════════════
// LIVE PREVIEW RENTANG JAM (Format Romawi)
// ══════════════════════════════════════════════
    ...
    function updatePreview() {
        ...
        if (mulai && selesai) {
            ...
            if (selesaiInt < mulaiInt) {
                ...
                previewEl.textContent = `Jam ${toRoman(mulaiInt)}-${toRoman(selesaiInt)}`;
...
// Konsisten dengan sipena-kelas.js yang simpan flat
// ══════════════════════════════════════════════
async function loadDaftarKelas() {
    try {
        const email = (currentUser.email || '').toLowerCase();
        const nama = (currentUser.nama || '').toLowerCase();
        
        // Baca dari Firestore collection 'sipena2' (format flat)
        const snap = await db.collection('sipena2').get();
        const allDocs = [];
        snap.forEach(doc => allDocs.push({ __key: doc.id, ...doc.data() }));
        
        // Filter: hanya kelas milik user ini (toleran email & nama)
        const kelasList = allDocs.filter(d => {
            if (d.type !== 'class') return false;
            const owner = (d.user_name || d.user_email || '').toLowerCase();
            return owner === email || owner === nama;
        });
        ...
        if (kelasList.length === 0) {
            ...
            console.warn('⚠️ Tidak ada kelas ditemukan untuk user:', email);
        } else {
...
// ══════════════════════════════════════════════
// SIMPAN JURNAL (dengan cek Auth terbaru)
// ══════════════════════════════════════════════
async function simpanJurnal(e) {
    ...
    try {
        // ✅ Cek Auth lagi (pastikan masih login saat submit)
        ...
        // Simpan NIP
        localStorage.setItem('sipelita_nip_' + sanitizeEmail(authUser.email), nip);
        ...
        // ✅ Simpan ke Firestore (auth.currentUser terisi → rules lolos)
